...
io.on("connection", (socket) => {
  ...
  socket.on("set-role", ({ roomId, role, team }) => {
    ...
    // დარწმუნდით, რომ "spymaster"-ის როლი მხოლოდ ერთ გუნდში იყოს
    if (role === "spymaster") {
      if (room.players.some(p => p.role === "spymaster" && p.team === team)) {
        return; // არ დავუშვებთ "spymaster"-ის როლს იმ გუნდში, სადაც უკვე არსებობს
      }
    }
